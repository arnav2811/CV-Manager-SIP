...
=======================================================================================
Uses the all-MiniLM-L6-v2 model to encode degree strings into 384-dim dense
vectors. Matches input against pre-encoded reference aliases via normalised dot
...
Note: Requires ~90 MB model download on first run (model cached after that).
"""
...
class NormalizerEmbeddings:
    """Semantic sentence-embedding 3-layer degree normalizer."""

    ENGINE_ID = "B-3_Embeddings"
    ...
    def _load_aliases(self) -> None:
        ...
        if not os.path.exists(degree_csv) or not os.path.exists(field_csv):
            print("[Embeddings] ERROR: Alias files not found.")
            return
        ...
        print(f"[Embeddings] Loaded {len(self.degree_aliases):,} degree aliases  |  "
    ...
    def _build_index(self) -> None:
        ...
        try:
            ...
            print("[Embeddings] WARNING: sentence-transformers not installed. "
        ...
        print(f"[Embeddings] Encoded {len(self.ref_choices):,} alias vectors")
    ...
    def layer2_fuzzy(self, cleaned: str,
                     ...
                     threshold_flag: float = 0.55) -> dict | None:
        ...
        try:
            ...
            print(f"[Embeddings] layer2 error: {exc}")
            return None
        ...
        if score >= threshold_auto:
            return {**base, "status": "fuzzy_matched",  "layer_used": "L2_Embeddings"}
        if score >= threshold_flag:
            return {**base, "status": "review_needed",  "layer_used": "L2_Embeddings"}
        return None
...
if __name__ == "__main__":
    ...
    while True:
        print("\n" + "=" * 60)
        print("  CV MANAGER · Embeddings Engine (B-3)   [standalone CLI]")
        print("=" * 60)
