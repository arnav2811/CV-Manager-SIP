...
**Unified advanced layer + local UI release** — Layer 2 fuzzy consensus and Layer 3 heuristic extraction have been integrated behind a single `L2_Unified` advanced layer. A lightweight local web UI has also been added so every system type can be tested from a browser without reintroducing FastAPI. Work completed by **Jai Gupta** with Codex assistance.

---

### 1. Layer 2 + Layer 3 Unified Layer (`engine_l2_l3_unified.py`)

- **Single post-L1 layer**: The orchestrator now routes `L1 exact lookup → L2_Unified advanced resolution` instead of running separate Layer 2 and Layer 3 passes.
- **Integrated resolution strategies**: `L2_Unified` first attempts fuzzy consensus, then runs heuristic extraction when fuzzy confidence is low, review-needed, or absent.
- **Clear audit metadata**: Results now expose `resolution_strategy` values such as `exact_lookup`, `fuzzy_consensus`, `heuristic_extraction`, and `unresolved`.
- **Full mode default**: The unified layer and orchestrator default to `full` mode so heuristic extraction is available unless a caller explicitly chooses a faster mode.

---

### 2. Orchestrator and CLI Routing Updates

- `engine_orchestrator.py` now keeps Layer 1 exact lookup separate and delegates all post-L1 work to `UnifiedAdvancedLayer`.
- `compare_all_engines()` now reports the unified advanced layer instead of a standalone L3-only branch.
- `app.py` menu option `[C]` now loads **L2+L3 Unified** instead of the old L2-only combined engine.
- Batch CSV output now includes `resolution_strategy` so training/evaluation exports show how each result was resolved.

---

### 3. Dataset Evaluation Routing

- `evaluate_datasets.py` now evaluates both L2 and L3 training datasets through the unified advanced layer.
- International degree-only datasets run through the full orchestrator in `full` mode.
- Evaluation reports now include the strategy field from `resolution_strategy` where available.

---

### 4. L3 False Positive Guard

- Tightened the permissive sentence-start heuristic that previously treated plain text like `"Some random text"` as a degree mention.
- Added explicit degree-signal and degree-prefix guards before S1 sentence extraction can return a raw mention.
- Generic mentions such as `Masters` and `diploma` are now canonicalized to stable degree names (`Master of Science`, `Post Graduate Diploma`) when extracted from conversational text.

---

### 5. Local Web UI (`poc/ui_server.py`)

- Added a dependency-light browser UI using Python's standard library HTTP server.
- UI surfaces include single normalization, batch normalization, all-engine comparison, dataset overview, engine availability, confidence, layer, strategy, and audit JSON.
- The UI covers all system types: L1 Exact, RapidFuzz, TF-IDF, Embeddings, L2+L3 Unified, Full Orchestrator, and L3 Heuristic.
- The dashboard runs locally at `http://127.0.0.1:8765` with `python poc/ui_server.py`.
...
|---|---|
| **Jai Gupta** | Requested and validated the unified Layer 2+3 system flow and complete local UI requirement |
| **Codex** | Implementation support for unified routing, UI server, documentation sync, and smoke validation |
...
| File | Change |
|---|---|
| `poc/engine_l2_l3_unified.py` | Updated — defaults to full mode and returns richer `mode` / `resolution_strategy` metadata |
| `poc/engine_orchestrator.py` | Updated — routes `L1 → L2_Unified` and removes active separate L2/L3 pass calls |
| `poc/app.py` | Updated — exposes `[C] L2+L3 Unified`; CSV export includes `resolution_strategy` |
| `poc/evaluate_datasets.py` | Updated — L2/L3 datasets evaluate through unified advanced layer |
| `poc/engine_l3.py` | Updated — false-positive guard and generic degree canonicalization |
| `poc/ui_server.py` | **New** — local browser UI for single, batch, compare, and dataset overview workflows |
| `README.md` | Updated — v3.7.0 architecture, quick-start, engine reference, and bug-fix notes |
