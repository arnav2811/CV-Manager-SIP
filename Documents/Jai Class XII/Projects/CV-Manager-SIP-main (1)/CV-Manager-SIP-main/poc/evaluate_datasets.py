...
def _load_engine(key: str, data_dir: str):
    ...
    from engine_l2_l3_unified  import UnifiedAdvancedLayer as Unified

    if key == "l1":
        return RF(data_dir)          # L1 exact-match only tested via full normalize()
    elif key == "unified":
        return Unified(data_dir, mode="full")
    else:                            # full / usa / uk / world
        return Orch(data_dir, mode="full")
...
def _normalize(engine, raw: str, engine_key: str) -> dict:
    """Run normalisation with a consistent engine API."""
    try:
...
def _evaluate(dataset_key: str, data_dir: str, limit: Optional[int],
    ...
    for raw, expected in rows:
        ...
        report_rows.append({
            ...
            "got":       got,
            "layer":     layer,
            "strategy":  r.get("resolution_strategy", ""),
            "conf":      round(conf, 4),
            "status":    status,
