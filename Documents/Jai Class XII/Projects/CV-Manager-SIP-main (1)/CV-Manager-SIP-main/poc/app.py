...
Engine roster
  ...
  [C]  L2+L3 Unified         - fuzzy consensus + heuristic extraction
  [D]  Full Orchestrator     - L1 + unified advanced layer
...
def _print_single_result(r: dict) -> None:
    ...
    if r.get("resolution_strategy"):
        print(f"  {'Resolution Strategy':<{W}}: {r['resolution_strategy']}")
...
def _load_engines(data_dir: str) -> dict:
    ...
    try:
        ...
        engines["[C] L2+L3 Unified"] = UnifiedAdvancedLayer(data_dir, mode="full")
    except Exception as exc:
        print(f"  [C] L2+L3 Unified unavailable: {exc}")
        engines["[C] L2+L3 Unified"] = None

    try:
        from engine_orchestrator import CVNormalizationOrchestrator
        engines["[D] Orchestrator"] = CVNormalizationOrchestrator(data_dir, mode="full")
    except Exception as exc:
            ...
            if save == "y":
                ...
                with open(out_path, "w", newline="", encoding="utf-8") as fh:
                    fieldnames = ["input", "canonical_degree", "canonical_field",
                                  "confidence", "status", "layer_used", "resolution_strategy",
                                  "fuzzy_score"]
...
def main() -> None:
    ...
    display_labels = [
        ...
        ("[C]    L2+L3 Unified",        "[C] L2+L3 Unified"),
