...
# (Updated for v3.7.0 — 1 July 2026)
...
>
> **v3.7.0 Update**: Layer 2 fuzzy consensus and Layer 3 heuristic extraction are now integrated behind one `L2_Unified` advanced layer (`engine_l2_l3_unified.py`). The orchestrator runtime is now `L1 exact lookup → L2_Unified`, and a lightweight local browser UI (`poc/ui_server.py`) has been added for single input, batch, compare, and dataset overview workflows.
...
#### Engine Option C: L2+L3 Unified (Fuzzy + Heuristic Advanced Layer) ⭐ Updated in v3.7.0
- **File**: `poc/engine_l2_l3_unified.py`
- **How**: Runs B-1 + B-2 + optional B-3 as fuzzy consensus first. If confidence is low, review-needed, or absent, the same layer runs the L3 heuristic extractor and returns one auditable `L2_Unified` result.
- **Speed**: Combined latency of all active engines (~2–110 ms depending on Embeddings availability).
- **Best at**: Maximum advanced-layer recall — fuzzy matches, typos, abbreviations, and conversational degree mentions are resolved behind one post-L1 layer.
...
## Version C: Full Unified Pipeline (Lookup + L2_Unified)

### What It Does
The **complete stack**. Inputs flow through Layer 1 and then the unified advanced layer via `engine_orchestrator.py`:
1. **Layer 1** — Exact dictionary lookup (instant resolution for known aliases).
2. **L2_Unified** — Combined fuzzy consensus (B-1 + B-2 + optional B-3) plus embedded pure-Python heuristic extraction for fully unstructured conversational text.
...
| Mode | L2 Engines | L3 Logic | Typical Latency |
|------|-----------|-----|----------------|
| `fast` | RapidFuzz only | No | 1–5 ms |
| `standard` | RapidFuzz + TF-IDF | No | 2–6 ms |
| `full` | RapidFuzz + TF-IDF + optional Embeddings | Embedded in L2_Unified | 50–120 ms |

### Layers Active
| Layer | Status |
|-------|--------|
| **L1 — Exact Lookup** | ✅ Active |
| **L2_Unified — Fuzzy + Heuristics** | ✅ Active |
| **L3 — NLP / Heuristics** | ✅ Embedded inside L2_Unified |
...
|-----------|-----------|-------------|-----------|
| **Layers** | L1 only | L1 + L2 | L1 + L2_Unified |
...
> **Start with Version B** using **Engine C (L2+L3 Unified)** for immediate production deployment. It provides the strongest balance of accuracy, speed, and resilience by combining fuzzy consensus with heuristic extraction in one auditable advanced layer. Use `standard` mode when only fuzzy matching is needed, and `full` mode when conversational inputs should be handled.
>
...
| Feature | B-1 RapidFuzz | B-2 TF-IDF | B-3 Embeddings | C L2+L3 Unified |
|---------|--------------|------------|----------------|------------|
| **Mechanism** | Token overlap + edit distance (combined) | Sub-string vector overlaps | Neural semantic vectors | Weighted vote + heuristic extraction |
